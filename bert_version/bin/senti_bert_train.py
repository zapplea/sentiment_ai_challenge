...
def train(conf, attr_model, senti_model):
    ...
    ## init senti net
    senti_sess.run(senti_model.init)
    average_loss = 0.0
    step = 0
    best_result = 0
    ...
    for step_i in range(conf["num_scan_data"]):
        for batch_index in range(train_batch_num):
            ...
            train_bert_emb = np.array(dg.generate_bert_sent_emb(train_review))
            senti_feed = {
                senti_model.review: train_bert_emb,
                senti_model.sent_att: curr_att,
                senti_model.senti_label: train_senti_label,
                senti_model.is_training: True
            }
            _, curr_loss, check = senti_sess.run([senti_model.g_updates, senti_model.loss, senti_model.rev_len], feed_dict=senti_feed)
