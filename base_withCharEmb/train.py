...
class SentiTrain:
    def __init__(self,config,df):
        ...
        self.config['train']['report_filePath'] = self.config['train']['report_filePath'] + '/baseWithChar_report_reg%s_lr%s.info' % \
                                               (str(self.config['model']['reg_rate']), str(self.config['model']['lr']))
        self.config['train']['sr_path'] = self.config['train']['sr_path'] + '/baseWithChar_ckpt_reg%s_lr%s/model.ckpt'%\
                                          (str(self.config['model']['reg_rate']), str(self.config['model']['lr']))
        ...
        with tf.Session(graph=graph,config=config) as sess:
            ...
            for epoch in range(self.config['train']['epoch_num']):
                ...
                if epoch%self.config['train']['mod']==0:
                    ...
                    print('Start Testing ...')
                    for _, senti_labels_data, sentences_data, sentences_char_data in dataset:
                        senti_loss_value,senti_pred_value = sess.run([senti_loss,senti_pred],feed_dict={senti_X:sentences_data, senti_Y:senti_labels_data, senti_char_X:sentences_char_data})
