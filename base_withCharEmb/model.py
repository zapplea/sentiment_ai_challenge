...
class Model:
    ...
    def build_senti_net(self):
        ...
        seq_len = self.layers.sequence_length(X_id,self.config['model']['padding_word_index'])
        ...
        char_seq_len = self.layers.sequence_length(char_X_id, self.config['model']['padding_char_index'])
        # (batch size*max sentence len,)
        char_seq_len = tf.reshape(char_seq_len,shape=(-1,))
